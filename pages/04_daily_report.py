...
def _get_daily_report_sqlite_db_path() -> str:
    """일일 리포트 포인트 조회와 거래 조회에 사용할 설정 SQLite 경로를 반환한다."""
...
with top1:
    st.markdown('<div class="page-title">🚨 오늘의 소비 알림장</div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="page-subtitle">일일 보고서는 분석보다 "내일 바로 바꿀 행동"에 집중합니다.</div>',
        unsafe_allow_html=True,
    )

with top2:
    if logged_in_member_id is None:
        f1, f2, f3 = st.columns([0.9, 1.27, 1.08])
