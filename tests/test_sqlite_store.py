...
from catcher_llm.config.raw_data_files import (
    RAW_TRANSACTIONS_CSV_FILENAME,
    RAW_TRANSACTIONS_CSV_FILENAME_ENV,
    RAW_USERS_CSV_FILENAME,
    RAW_USERS_CSV_FILENAME_ENV,
)
...
def _write_seed_csvs(csv_dir: Path, *, encoding: str = "utf-8") -> None:
    ...
    (csv_dir / RAW_USERS_CSV_FILENAME).write_text(
        "\n".join(
            [
                "id,name,age,occupation,gender,annual_income,region,persona,personal_score,saving_goal_text,target_max_spending_amount",
                "1,김토스,29,개발자,남성,7000,서울,절약형,88,비상금 300만원 만들기,1200000",
                "2,이캐처,33,기획자,여성,6500,부산,안정형,72,월세 비상금 만들기,1400000",
            ]
        ),
        encoding=encoding,
    )
    (csv_dir / RAW_TRANSACTIONS_CSV_FILENAME).write_text(
        "\n".join(
            [
                ...
                "100,1,12000,2026-04-01 09:00:00,커피,카페,False,0,False,APPROVED,False,식음료,OFFLINE",
                "101,1,45000,2026-04-02 18:30:00,마트,대형마트,False,0,False,APPROVED,False,생활,OFFLINE",
...
def test_ensure_user_database_seeds_sqlite_from_csv(tmp_path: Path) -> None:
    ...
    assert settings.members_csv_path.name == RAW_USERS_CSV_FILENAME
    assert settings.consumption_csv_path.name == RAW_TRANSACTIONS_CSV_FILENAME
...
def test_ensure_user_database_rebuilds_when_seed_counts_do_not_match_metadata(
    tmp_path: Path,
) -> None:
    """시드 메타데이터가 최신이어도 SQLite 행 수가 다르면 CSV 기준으로 재생성되는지 검증한다."""
    settings = _make_settings(tmp_path)
    ensure_user_database(settings=settings)
    ...
    assert result.user_count == 2
    assert _fetch_sqlite_cell(settings.sqlite_db_path, "users", "name", row_id=2) == "이캐처"
...
def test_settings_uses_raw_csv_filename_environment_variables(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """raw CSV 파일명 환경변수가 설정되면 Settings 경로가 해당 파일명을 사용한다."""
    monkeypatch.setenv(RAW_USERS_CSV_FILENAME_ENV, "custom_users.csv")
    monkeypatch.setenv(RAW_TRANSACTIONS_CSV_FILENAME_ENV, "custom_transactions.csv")
    settings = Settings(raw_data_dir=tmp_path / "raw")

    assert settings.members_csv_path.name == "custom_users.csv"
    assert settings.consumption_csv_path.name == "custom_transactions.csv"
...
def test_authenticate_user_and_load_transactions_from_sqlite(tmp_path: Path) -> None:
    ...
    assert profile["saving_goal_text"] == "비상금 300만원 만들기"
    assert len(transactions) == 2
