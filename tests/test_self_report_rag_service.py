...
class SelfReportRagServiceTests(unittest.TestCase):
    def test_generate_self_report_rag_reply_normalizes_report_fact_answers(self) -> None:
        """소비 자기진단 리포트 RAG 서비스가 보고서 수치와 순위를 직접 답변으로 정규화하는지 검증한다."""
        ...
        with (
            patch("catcher_llm.services.rag.self_report.get_rag_pipeline_config") as get_config,
            patch("catcher_llm.services.rag.self_report.get_self_report_prompt"),
            patch(
                "catcher_llm.services.rag.self_report.generate_rag_reply",
    ...
    def test_get_self_report_prompt_requests_fact_style_answer(self) -> None:
        """소비 자기진단 리포트 프롬프트가 보고서 기반 한 문장 사실형 답변을 요구하는지 검증한다."""
        ...
        self.assertIn("소비 자기진단 리포트", messages[0].content)
        self.assertIn("답변은 한 문장만 쓴다", messages[0].content)
        self.assertIn("201812 회원월당 소비성 금액은 얼마인가?", messages[1].content)

    def test_generate_self_report_rag_reply_uses_tighter_default_top_k_and_prompt(self) -> None:
        """소비 자기진단 리포트 RAG 서비스가 전용 프롬프트와 기본 top_k=2를 사용하는지 검증한다."""
        with (
            patch("catcher_llm.services.rag.self_report.get_rag_pipeline_config") as get_config,
            patch("catcher_llm.services.rag.self_report.get_self_report_prompt") as get_prompt,
            patch("catcher_llm.services.rag.self_report.generate_rag_reply") as generate_reply,
        ):
            get_config.return_value.raw_data_dir = "raw"
            get_config.return_value.source_files = ["doc.pdf"]
            ...
            generate_self_report_rag_reply("질문")
