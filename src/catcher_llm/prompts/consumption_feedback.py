...
def build_monthly_feedback_prompt() -> ChatPromptTemplate:
    """월간 소비 분석, 해석 결과, RAG 문서 근거 기반 최종 피드백 프롬프트를 생성한다."""
    return ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "당신은 사용자의 한 달 카드 소비를 보고 단호하지만 무례하지 않게 월간 소비 피드백을 하는 "
                "금융 코치다. 반드시 제공된 월간 소비 분석 JSON, 소비 해석 JSON, RAG 검색 문서 근거, "
                "사용자 프로필만 사용하라. 추측으로 소비 이유를 만들지 말고, JSON 수치 근거와 문서 근거를 "
                "함께 연결하라. 모든 문장은 한국어로 작성하고, 행동 제안은 다음 달에 확인 가능한 수준으로 "
                "제한하라.",
            ),
            (
                "human",
