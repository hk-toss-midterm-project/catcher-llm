...
def _extract_contexts(rag_response: Any) -> list[str]:
    """RAG 응답 객체에서 ragas 평가용 컨텍스트 문자열 목록을 추출한다."""
...
def _extract_answer(rag_response: Any) -> str:
    """RAG 응답 객체에서 평가 대상 답변 문자열을 추출한다."""
...
def build_saving_tips_ragas_dataset(settings: Settings | None = None) -> Dataset:
    """절약 팁 질문 세트와 RAG 응답으로 ragas 평가용 데이터셋을 구성한다."""
    ...
    for question in questions:
        rag_response = generate_saving_tips_rag_reply(question, settings=settings)
...
def run_saving_tips_ragas_eval(settings: Settings | None = None) -> EvaluationResult:
    """프로젝트 설정의 모델로 절약 팁 ragas 평가를 실행한다."""
    config = settings or get_settings()
    dataset = build_saving_tips_ragas_dataset(settings=config)
...
def save_saving_tips_ragas_result(
    output_path: str = "saving_tips_ragas_result.csv",
    settings: Settings | None = None,
) -> DataFrame:
    """절약 팁 ragas 평가 결과를 CSV로 저장하고 데이터프레임으로 반환한다."""
