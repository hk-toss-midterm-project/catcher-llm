...
def _build_document_daily_metrics(user_data: UserSpendingData) -> list[SpendingMetric]:
    ...
    return [
        ...
        make_spending_metric(
            "일일 잔여 예산",
            daily_metrics.daily_remaining_budget,
            "KRW",
            "daily_metrics.daily_remaining_budget",
            "분석 기준일에 배정된 예산에서 오늘 소비를 제외하고 남은 금액",
        ),
        make_spending_metric(
            "일일 예산 초과액",
            daily_metrics.daily_overspend_amount,
            "KRW",
            "daily_metrics.daily_overspend_amount",
            "분석 기준일에 배정된 예산을 초과해 사용한 금액",
        ),
        make_spending_metric(
            "일 환산 소득 대비 소비율",
            ...
            "percent",
            "daily_metrics.daily_income_usage_rate_percent",
            "월소득을 해당 월 일수로 나눈 일 소득 대비 오늘 소비 비율",
        ),
        make_spending_metric(
            "월 누적 목표 사용률",
            daily_metrics.month_to_date_budget_usage_rate_percent,
            "percent",
            "daily_metrics.month_to_date_budget_usage_rate_percent",
            "월초부터 분석 기준일까지의 누적 소비가 월 목표 소비 한도에서 차지하는 비율",
        ),
        make_spending_metric(
            "월말 예상 소비",
            daily_metrics.projected_monthly_spending,
            "KRW",
            "daily_metrics.projected_monthly_spending",
            "현재 월 누적 소비 페이스를 월말까지 유지할 때 예상되는 월 소비 금액",
        ),
        make_spending_metric(
            "월말 예상 목표 사용률",
            ...
            "percent",
            "daily_metrics.projected_monthly_budget_usage_rate_percent",
            "월말 예상 소비가 월 목표 소비 한도에서 차지하는 비율",
        ),
        make_spending_metric(
            "월말까지 남은 하루 허용 소비",
            daily_metrics.required_daily_budget_until_month_end,
            "KRW",
            "daily_metrics.required_daily_budget_until_month_end",
            "월 목표를 지키기 위해 분석일 이후 남은 기간에 하루 평균 사용할 수 있는 금액",
        ),
        make_spending_metric(
            "무소비일 여부",
