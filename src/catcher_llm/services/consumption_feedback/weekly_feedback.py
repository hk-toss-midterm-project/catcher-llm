...
def _build_weekly_core_metrics(weekly_data: WeeklySpendingData) -> list[SpendingMetric]:
    ...
    return [
        ...
        make_spending_metric(
            "주간 잔여 예산",
            weekly_metrics.weekly_remaining_budget,
            "KRW",
            "weekly_metrics.weekly_remaining_budget",
            "분석 주간에 배정된 예산에서 실제 소비를 제외하고 남은 금액",
        ),
        make_spending_metric(
            "주간 예산 초과액",
            weekly_metrics.weekly_overspend_amount,
            "KRW",
            "weekly_metrics.weekly_overspend_amount",
            "분석 주간에 배정된 예산을 초과해 사용한 금액",
        ),
        make_spending_metric(
            "주 환산 소득 대비 소비율",
            ...
            "percent",
            "weekly_metrics.weekly_income_usage_rate_percent",
            "월소득을 분석 주간 날짜 수만큼 환산한 금액 대비 주간 소비 비율",
        ),
        make_spending_metric(
            "주간 예산 소진 배율",
            weekly_metrics.weekly_budget_burn_rate,
            "ratio",
            "weekly_metrics.weekly_budget_burn_rate",
            "분석 주간 소비 금액을 주간 배정 예산으로 나눈 배율",
        ),
        make_spending_metric(
            "월 누적 목표 사용률",
            ...
            "percent",
            "weekly_metrics.month_to_date_budget_usage_rate_percent",
            "분석 주 종료일까지의 월 누적 소비가 월 목표 소비 한도에서 차지하는 비율",
        ),
        make_spending_metric(
            "주간 페이스 기준 월말 예상 소비",
            ...
            "KRW",
            "weekly_metrics.projected_monthly_spending_from_weekly_pace",
            "분석 주간 일평균 소비를 월말까지 유지할 때 예상되는 월 소비 금액",
        ),
        make_spending_metric(
            "주말 과소비 지수",
        ...
        fb_rows = list(
            _fb_session.scalars(
                ...
                .where(
                    ...
                    UserFeedbackMemoryModel.period_type == _WEEKLY_MEMORY_PERIOD_TYPE,
                )
...
def generate_weekly_feedback(
    *,
    ...
    memory_context: DailyFeedbackMemoryContext | None = None
    try:
        weekly_payload = run_timed_feedback_step(
            step_key="weekly_analysis",
            step_name="주간 소비 분석 JSON 생성",
            detail="SQLite transactions 조회와 pandas 지표 계산",
            timing_callback=timing_callback,
            operation=lambda: build_weekly_consumption_analysis_json(
                member_id=member_id,
                week_start=start_day,
                week_end=end_day,
                settings=config,
        ...
        weekly_data = run_timed_feedback_step(
            step_key="parse_weekly_analysis",
            step_name="주간 분석 모델 검증",
            detail="분석 JSON을 WeeklySpendingData Pydantic 모델로 변환",
            timing_callback=timing_callback,
            operation=lambda: parse_weekly_spending_data(weekly_payload),
        )
        ...
        memory_context = run_timed_feedback_step(
            ...
            detail="SQLite user_memories와 최근 weekly session 조회",
            timing_callback=timing_callback,
            operation=lambda: load_weekly_feedback_memory_context(
                member_id=member_id,
                week_start=start_day,
                settings=config,
            ),
        ...
        run_timed_feedback_step(
            ...
            timing_callback=timing_callback,
            operation=lambda: save_weekly_feedback_session(
                member_id=member_id,
                week_start=start_day,
        ...
        run_timed_feedback_step(
            ...
            detail=f"최근 최대 {_DEFAULT_WEEKLY_MEMORY_SESSION_LIMIT}개 세션을 LLM으로 요약",
            timing_callback=timing_callback,
            operation=lambda: _refresh_weekly_user_memory_if_possible(
                member_id=member_id,
                settings=config,
            ),
        )
